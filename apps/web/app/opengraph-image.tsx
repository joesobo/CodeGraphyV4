...
export default async function OpenGraphImage(): Promise<ImageResponse> {
  const [brandMarkSvg, font, fontBold, oceanImage] = await Promise.all([
  ...
  return new ImageResponse(
    (
      <div style={{ background: '#061722', color: '#fff', display: 'flex', fontFamily: 'Fira Code', height: '100%', overflow: 'hidden', position: 'relative', width: '100%' }}>
        ...
        <div style={{ background: 'rgba(3,18,29,.68)', display: 'flex', inset: 0, position: 'absolute' }} />
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', justifyContent: 'space-between', padding: 64, position: 'relative', width: 780 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div style={{ color: '#88b1ff', display: 'flex', fontSize: 17, fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase' }}>Relationship Graph</div>
            ...
            <div style={{ color: 'rgba(255,255,255,.78)', display: 'flex', fontSize: 22, lineHeight: 1.45 }}>Local Indexing for developers, the VS Code extension, the Core CLI, and shell-capable agents.</div>
          </div>
        </div>
      </div>
    ),
