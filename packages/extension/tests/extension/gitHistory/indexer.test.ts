...
describe('gitHistory/indexer', () => {
  ...
  it('indexes topological branch history against each commit first parent graph', async () => {
    const graphRoot = createGraph('root');
    const graphMain = createGraph('main');
    const graphFeature = createGraph('feature');
    const graphMerge = createGraph('merge');
    const commits = [
      createCommit('root'),
      createCommit('main-1', ['root']),
      createCommit('feature-1', ['root']),
      createCommit('merge-1', ['main-1', 'feature-1']),
    ];
    const dependencies = {
      analyzeDiffCommit: vi.fn()
        .mockResolvedValueOnce(graphMain)
        .mockResolvedValueOnce(graphFeature)
        .mockResolvedValueOnce(graphMerge),
      analyzeFullCommit: vi.fn(async () => graphRoot),
      getCommitList: vi.fn(async () => commits),
      persistCachedCommitState: vi.fn(),
      writeCachedGraphData: vi.fn(),
    };
  ...
  it('falls back to full analysis when the first parent is outside the indexed range', async () => {
    const graphFirstVisible = createGraph('first-visible');
    const graphSecond = createGraph('second');
    const commits = [
      createCommit('first-visible', ['missing-parent']),
      createCommit('second', ['first-visible']),
    ];
    const dependencies = {
      analyzeDiffCommit: vi.fn(async () => graphSecond),
      analyzeFullCommit: vi.fn(async () => graphFirstVisible),
      getCommitList: vi.fn(async () => commits),
      persistCachedCommitState: vi.fn(),
      writeCachedGraphData: vi.fn(),
    };
    ...
    expect(dependencies.analyzeFullCommit).toHaveBeenCalledWith(
      'first-visible',
      expect.any(AbortSignal),
    );
    expect(dependencies.analyzeDiffCommit).toHaveBeenCalledWith(
