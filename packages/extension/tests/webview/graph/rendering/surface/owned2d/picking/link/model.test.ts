...
} from '../../../../../../../../src/webview/components/graph/rendering/surface/owned2d/picking/link/model';
...
describe('owned graph link picking', () => {
  it('picks straight links within a zoom-independent screen distance', () => {
    ...
    picker.rebuild([straight]);

    expect(picker.pick({ x: 50, y: 5 }, 1)?.link).toBe(straight);
    expect(picker.pick({ x: 50, y: 7 }, 1)).toBeUndefined();
    expect(picker.pick({ x: 50, y: 0.5 }, 10)?.link).toBe(straight);
    expect(picker.pick({ x: 50, y: 1 }, 10)).toBeUndefined();
  });
  ...
  it('follows curved link geometry and returns the nearest overlap', () => {
    ...
    const straight = link();
    const picker = new OwnedGraphLinkPicker();
    picker.rebuild([straight, upward]);
