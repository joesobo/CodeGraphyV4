...
import { createGraphViewProviderFileActionMethods } from '../../../../src/extension/graphView/provider/fileActions';

describe('graphView/provider/fileActions', () => {
  afterEach(() => {
    vi.doUnmock('vscode');
...
async function createDefaultDependencyHarness() {
  ...
  vi.doMock('../../../../src/extension/graphView/provider/fileNavigation', () => ({
  ...
  const { createGraphViewProviderFileActionMethods: createMethods } = await import(
    '../../../../src/extension/graphView/provider/fileActions'
  );
