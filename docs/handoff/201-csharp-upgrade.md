...
- State: Orchestrator setup is in progress; the grill-before-implementation gate is active.
...
- Draft PR: pending first push from this restart
...
- Trello card `rSYGlC3d` is open, titled `C# Upgrade`, and was still in `Todo` during setup verification.
