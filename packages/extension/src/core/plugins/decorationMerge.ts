/**
 * @fileoverview Pure merge functions for node and edge decorations.
 * Extracted from DecorationManager to keep merge logic independently testable.
 * @module core/plugins/decorationMerge
 */

import type { NodeDecoration, EdgeDecoration, TooltipSection } from './DecorationManager';

export type { NodeDecoration, EdgeDecoration, TooltipSection };

/**
 * Merge multiple node decorations (already sorted by priority descending).
 * First-set-wins per property; tooltip sections are concatenated from all.
 */
