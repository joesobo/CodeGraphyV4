...
vi.mock('../../../src/extension/graphViewPhysics', () => ({
...
vi.mock('../../../src/extension/graphView/physicsConfig', () => ({
...
describe('graphView/providerPhysicsSettingsMethods default dependencies', () => {
  beforeEach(() => {
  ...
  it('updates physics settings through the default config-target helpers', async () => {
    ...
    const options = mocks.updatePhysicsSetting.mock.calls[0]?.[2] as {
      getConfiguration(): unknown;
      getConfigTarget(): unknown;
    };
    expect(options.getConfiguration()).toBe(mocks.configuration);
    expect(options.getConfigTarget()).toBe('workspace-target');
  ...
  it('resets physics settings through the default config-target helpers', async () => {
    ...
    const options = mocks.resetPhysicsSettings.mock.calls[0]?.[0] as {
      getConfiguration(): unknown;
      getConfigTarget(): unknown;
    };
    expect(options.getConfiguration()).toBe(mocks.configuration);
