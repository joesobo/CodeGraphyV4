...
test('extension changelog stays aligned with the published extension version', () => {
  ...
  const changelog = readFileSync(path.join(repoRoot, 'packages', 'extension', 'CHANGELOG.md'), 'utf8');
  const topVersion = changelog.match(/^##\s+([0-9]+\.[0-9]+\.[0-9]+)/m)?.[1];

  assert.ok(topVersion, 'expected packages/extension/CHANGELOG.md to start with a version heading');
  assert.equal(
    topVersion,
    extensionPackageJson.version,
    'expected packages/extension/CHANGELOG.md top version to match packages/extension/package.json version',
  );
});
