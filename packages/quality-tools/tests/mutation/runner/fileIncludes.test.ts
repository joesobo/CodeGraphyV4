...
describe('fileIncludes', () => {
  it('includes mirrored and broad fallback globs for ordinary source files', () => {
    ...
    expect(includes).toContain('packages/extension/__tests__/**/graphViewProvider/**/*.test.ts');
  });
  ...
  it('skips broad fallback globs for generic basenames', () => {
    ...
    expect(includes).not.toContain('packages/extension/__tests__/**/runtime/**/*.test.ts');
  });
  ...
  it('includes detector and camel-cased rule tests for source rules', () => {
    ...
    expect(includes).toContain('packages/plugin-csharp/__tests__/**/ruleDetectors.test.ts');
    expect(includes).toContain('packages/plugin-csharp/__tests__/**/*Detectors.test.tsx');
  });
});
