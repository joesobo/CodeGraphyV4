...
describe('pluginHost/api/containers', () => {
  ...
  it('creates slot containers and moves them into attached hosts', () => {
    const slotContainers = new Map<string, Map<'toolbar', HTMLDivElement>>();
    const slotHosts = new Map<'toolbar', HTMLDivElement>();
  ...
  it('reuses an existing slot container and can attach a host after the container already exists', () => {
    const slotContainers = new Map<string, Map<'toolbar' | 'timeline', HTMLDivElement>>();
    const slotHosts = new Map<'toolbar' | 'timeline', HTMLDivElement>();
    ...
    attachSlotHost('toolbar', host, slotContainers as never, slotHosts as never);
    getOrCreateSlotContainer('plugin-a', 'timeline', slotContainers as never, slotHosts as never);
