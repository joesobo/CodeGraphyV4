...
describe('owned graph WASM physics preparation', () => {
  ...
  it('compiles the emitted module through the streaming path', async () => {
    ...
    const { prepareOwnedGraphPhysics } = await import('../../../src/webview/wasm/ownedGraphPhysics');
    const registry = await import('../../../src/webview/components/graph/rendering/surface/owned2d/physics/wasm/module');
    await prepareOwnedGraphPhysics();
  ...
  it('reports an unsuccessful WASM asset request explicitly', async () => {
    ...
    await expect(prepareOwnedGraphPhysics()).rejects.toThrow(
  ...
  it('shares one fetch and compilation across concurrent callers', async () => {
    ...
    const { prepareOwnedGraphPhysics } = await import('../../../src/webview/wasm/ownedGraphPhysics');
    const first = prepareOwnedGraphPhysics();
    const second = prepareOwnedGraphPhysics();
