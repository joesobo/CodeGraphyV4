...
describe('extension/repoSettings/store/model/nestedValues', () => {
  it('reads and reports nested values through explicit keys', () => {
    ...
    expect(getNestedValue(settings, 'physics.damping')).toBe(0.7);
    expect(getNestedValue(settings, 'physics.chargeRange')).toBeUndefined();
    expect(getNestedValue(settings, 'filterPatterns.0')).toBeUndefined();
    expect(getNestedValue({ physics: 2 }, 'physics.damping')).toBeUndefined();
    expect(hasNestedValue(settings, 'filterPatterns')).toBe(true);
    expect(hasNestedValue(settings, 'filterPatterns.0')).toBe(false);
    expect(hasNestedValue({ physics: 2 }, 'physics.damping')).toBe(false);
    expect(hasNestedValue(settings, 'physics.chargeRange')).toBe(false);
  });
