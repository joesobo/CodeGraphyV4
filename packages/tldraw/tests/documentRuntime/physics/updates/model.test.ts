...
describe('tldraw physics shape updates', () => {
  it('realigns a companion after it is explicitly moved into its node frame', () => {
    const node = {
      id: 'shape:a',
    ...
    const geometryHost = {
      getPointInParentSpace: (
        _shape: typeof node | typeof icon,
        point: { x: number; y: number },
      ) => ({ x: point.x - 400, y: point.y - 200 }),
      getShapePageBounds: () => ({ h: 120, w: 120, x: 440, y: 340 }),
    };
    ...
    expect(createShapeUpdates(model, engine)).toEqual([
      ...
      { id: 'icon:a', parentId: 'shape:frame', type: 'image', x: 72, y: 172 },
    ]);
  });

  it('moves an edge into a shared native frame with local geometry', () => {
    const nodes = [
      {
        id: 'shape:a',
        parentId: 'shape:frame',
    ...
    const engine = {
      nodeIds: ['a', 'b'],
      x: Float32Array.of(96, 136),
      y: Float32Array.of(60, 100),
    } as unknown as GraphLayoutEngine;
    const geometryHost = {
      getPointInParentSpace: (
        shape: typeof nodes[number] | typeof edge,
        point: { x: number; y: number },
      ) => shape.parentId === 'shape:frame'
        ? { x: point.x - 400, y: point.y - 200 }
        : point,
      getShapePageBounds: (shape: typeof nodes[number]) => ({
        h: 120,
        w: 120,
        x: shape.x + 400,
        y: shape.y + 200,
