...
export interface GraphSurfaceSharedProps {
  ...
  onNodeDragEnd(this: void, node: NodeObject, translate: { x: number; y: number }): void;
...
export interface BuildSharedGraphPropsOptions {
  ...
  onNodeDragEnd(this: void, node: FGNode, translate: { x: number; y: number }): void;
...
export function buildSharedGraphProps(
  ...
  return {
    ...
    onNodeDrag: (node, translate) => options.onNodeDrag(node as FGNode, translate),
    onNodeDragEnd: (node, translate) => options.onNodeDragEnd(node as FGNode, translate),
