...
} from '../../../src/extension/workspaceAnalyzer/discovery';

describe('workspaceAnalyzer/discovery', () => {
  it('merges default, plugin, and user filters before discovery', async () => {
    ...
    await discoverWorkspaceAnalyzerFiles(
      { discover },
      '/workspace',
      {
        include: ['**/*'],
  ...
  it('formats the limit-reached warning message', () => {
    expect(formatWorkspaceAnalyzerLimitReachedMessage(27, 10)).toBe(
