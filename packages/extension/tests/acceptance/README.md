...
`packages/extension/tests/playwright-vscode/generated/`. They are ignored build
artifacts generated by `generate:acceptance`, and they should not be edited by
hand.

The generated JSON IR lives under
`packages/extension/tests/playwright-vscode/generated-ir/`. Advisory DRY
reports are written under `packages/extension/reports/acceptance-dry/`. All of
these outputs are ignored by git. The generated Playwright entrypoints load JSON
IR through a shared generated runtime, so the committed `.feature` files stay
thin while preserving source line metadata and Playwright `test.step` reporting.
The DRY reports surface repeated step patterns and scenario shapes for review;
they do not rewrite specs or change test behavior.
