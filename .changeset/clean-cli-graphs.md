...
- Make CLI help self-contained for agents. Return stable JSON envelopes, resumable pagination, bounded-path completeness, and one-off Filter, Node Type, and Edge Type projections.
...
- Index Nodes, Symbols, and Edges once regardless of Graph Scope. Extend doctor with cache metadata and record counts, keep a new index fresh when configured Plugins are unavailable, and persist the complete extension graph.
