...
- Adjacent Timeline Snapshots now send `diffGraphData` patches instead of full `COMMIT_GRAPH_DATA` graph payloads. The webview applies those patches in place without incrementing `graphResetVersion`, preserving live object identity. Task 4.4 remains open because the separate user-toggleable revision-diff edge class itself is not yet defined in the current graph contract.
