...
- inherit edges for embedded Go structs/interfaces
...
- `internal/service/service.go` imports `internal/model` and `strings`; it defines `Runner`, `TaskRunner`, `Status`, `DefaultStatus`, `NewTaskRunner`, and the `TaskRunner.Run` method. `TaskRunner` embeds `model.Audited`.
...
- Interface: `Runner`, `Notifier`
- Type: `Status`
- Constant: `startupMessage`, `DefaultStatus`
