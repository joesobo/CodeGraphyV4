...
| `example-vue` | A Vue 3 workspace with `<script setup lang="ts">`, normal `<script lang="ts">`, explicit `.vue` imports, extensionless component imports, composables, and type-only imports. Before the Vue plugin exists, use it to inspect which `.vue` nodes and connections are missing from the baseline graph. |
