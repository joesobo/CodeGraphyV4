...
import { createWorkspaceAnalyzerAnalysisSource, createWorkspaceAnalyzerRebuildSource } from '../../../src/extension/workspaceAnalyzer/analysisSource';

describe('workspaceAnalyzer/analysisSource', () => {
  it('adapts analyzer state and delegates for the analysis runner', async () => {
    ...
    const source = createWorkspaceAnalyzerAnalysisSource(analyzer as never);
  ...
  it('adapts cached state for rebuild-only graph updates', () => {
    ...
    const source = createWorkspaceAnalyzerRebuildSource(analyzer as never);
