...
export function integrateGraphLayout(
  ...
  for (let index = 0; index < state.x.length; index += 1) {
    ...
    state.vx[index] += -state.x[index] * config.centerForce * alpha * 0.001;
    state.vy[index] += -state.y[index] * config.centerForce * alpha * 0.001;
    ...
    state.vx[index] *= config.damping;
    state.vy[index] *= config.damping;
