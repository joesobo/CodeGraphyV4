...
describe('extension/repoSettings/store', () => {
  ...
  it('creates .codegraphy/settings.json from defaults instead of seeding legacy configuration', () => {
    ...
    expect(store.get('physics.linkDistance', 0)).toBe(80);
  ...
  it('reports explicit and parent-child matches through affectsConfiguration', async () => {
    ...
    await store.update('physics.chargeRange', 300);
    ...
    expect(events[1].affectsConfiguration('codegraphy.physics.chargeRange')).toBe(true);
  ...
  it('inspects default and workspace values for nested keys', async () => {
    ...
    expect(store.inspect<number>('physics.linkDistance')).toEqual({
      defaultValue: 80,
      workspaceValue: 80,
    });

    await store.update('physics.linkDistance', 120);
