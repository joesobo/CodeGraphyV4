...
test('VS Code graph view runner can skip the analyze idle wait for live-update markers', async (t) => {
  const moduleUrl = pathToFileURL(
    path.resolve('scripts/performance/measure-vscode-graph-view.mjs'),
  ).href;
  const { measureLiveUpdateTransition } = await import(moduleUrl);
  const workspaceRoot = await mkdtemp(path.join(os.tmpdir(), 'codegraphy-live-update-no-wait-'));
  t.after(() => rm(workspaceRoot, { recursive: true, force: true }));
  ...
  try {
    ...
    assert.ok(
      markerSeenAt < analyzeCompletedAt,
      `marker waited for analyze completion: marker=${markerSeenAt} analyze=${analyzeCompletedAt}`,
    );
  } finally {
    stopped = true;
    await observer;
