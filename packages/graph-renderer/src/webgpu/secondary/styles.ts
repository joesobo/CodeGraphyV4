...
function packSecondaryNodeStyles(
  ...
  styles.nodeStyleValues = new Float32Array(frame.nodes.length * NODE_STYLE_FLOATS);
...
function packSecondaryLinkStyles(
  ...
  const cached = createLinkStyles({
    ...frame,
    getLinkColor: secondary.getLinkColor,
    getLinkOpacity: secondary.getLinkOpacity,
    getLinkWidth: secondary.getLinkWidth,
  });
  styles.linkStyleValues = new Float32Array(
    primary.renderedLinkCount * LINK_INSTANCE_STYLE_FLOATS,
  );
