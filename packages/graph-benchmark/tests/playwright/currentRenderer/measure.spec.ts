...
import { selectSyntheticDragTarget } from '../../../src/harness/driver';
...
test('captures rendered frame times during a fixed synthetic node drag', async ({ page }) => {
  const fixture = createSyntheticFixture('1k', 307);
  const server = await startGraphBenchmarkServer(fixture);
  ...
  try {
    ...
    const result = await runCurrentRendererSyntheticDrag(page, targetNodeId);
    ...
    expect(result.draggedNodeId).toBe(targetNodeId);
