...
`@codegraphy/plugin-godot` is the first structured-analysis showcase. It keeps one npm package, but routes GDScript statements, text-resource tags, and `project.godot` settings through plugin-owned structured helpers before emitting the same relationship and Symbol Node output. The current implementation does not depend on a Godot AST package yet; parser packages such as `tree-sitter-gdscript` and `tree-sitter-godot-resource` are the next upgrade path once relationship parity is stable. This lets the plugin gradually move expensive line scanning behind Godot-aware parsing without introducing a Godot LSP process or VS Code-specific dependency.
