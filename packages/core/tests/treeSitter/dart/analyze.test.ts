  ...
  it('extracts Dart import relationships, simple inheritance, and useful symbols', async () => {
    ...
    expect(result?.relations).toEqual(expect.arrayContaining([
      expect.objectContaining({
        ...
        target: { kind: 'file', path: path.join(workspaceRoot, 'lib/model/user.dart'), pathKind: 'absolute', specifier: '../model/user.dart' },
      }),
      expect.objectContaining({
        edgeType: 'import',
        pluginId: 'codegraphy.treesitter',
        sourceId: 'codegraphy.treesitter:import',
        from: { kind: 'file', filePath: runnerPath },
        target: { kind: 'file', path: path.join(workspaceRoot, 'lib/model/profile.dart'), pathKind: 'absolute', specifier: 'package:sample_app/model/profile.dart' },
      }),
      expect.objectContaining({
        edgeType: 'import',
        pluginId: 'codegraphy.treesitter',
        sourceId: 'codegraphy.treesitter:import',
        from: { kind: 'file', filePath: runnerPath },
        target: { kind: 'unresolved', specifier: 'dart:convert' },
      }),
      expect.objectContaining({
        edgeType: 'inherit',
        pluginId: 'codegraphy.treesitter',
