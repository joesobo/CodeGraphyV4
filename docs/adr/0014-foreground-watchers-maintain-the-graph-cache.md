...
The CLI exposes that updater as foreground `codegraphy watch [-C workspace]`. It installs shutdown handling and subscribes before initial synchronization, uses Parcel's native recursive watcher, ignores cache artifacts at every `.codegraphy` path segment except the root settings file, and retains every startup event—including settings changes—for a serialized follow-up update. It emits bounded JSON Lines envelopes and drains pending changes on `SIGINT` or `SIGTERM`, including interrupts received during initial synchronization. Query commands remain separate one-shot processes.
...
Graph Cache writes use operation-scoped cross-process coordination through a separate SQLite coordinator beside `graph.sqlite`. Each writer holds an exclusive transaction only for the cache-write operation; SQLite serializes contenders atomically and releases ownership when a process terminates, without check-then-delete stale-lock recovery. Corrupt-database repair and retry remain under that same ownership even when `graph.sqlite` itself cannot be opened. Graph Cache connections also wait up to five seconds for short-lived reader contention. Incremental analyses verify source content immediately before commit and retry if another writer's newer edit superseded them. This supports simultaneous CLI watchers and independent readers without an exclusive long-lived watcher owner or heartbeat.
