...
export const INTERACTIVE_COOLDOWN_TICKS = 0;
export const INITIAL_INTERACTIVE_COOLDOWN_TICKS = 60;
...
function hasSettledNodePositions(nodes: readonly NodeObject[]): boolean {
  return nodes.length > 0
    && nodes.every((node) => Number.isFinite(node.x) && Number.isFinite(node.y));
}

function resolveCooldownTicks(options: Pick<BuildSharedGraphPropsOptions, 'graphData' | 'timelineActive'>): number {
  if (options.timelineActive) {
    return TIMELINE_COOLDOWN_TICKS;
  }

  return hasSettledNodePositions(options.graphData.nodes)
...
export function buildSharedGraphProps(
  ...
  return {
    ...
    d3AlphaDecay: 0.0228,
    warmupTicks: 0,
    cooldownTicks: resolveCooldownTicks(options),
    nodeId: 'id',
