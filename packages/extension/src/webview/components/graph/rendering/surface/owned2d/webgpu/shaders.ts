...
fn vertexMain(
  ...
  if (vertexIndex >= 24u) {
    ...
    let corner = vertexIndex - 24u;
    let graphPosition = curvePoint(graphSource, graphDestination, control, 0.72);
    let graphTangent = curveTangent(graphSource, graphDestination, control, 0.72);
