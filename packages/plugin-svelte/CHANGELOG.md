...
Users can install and enable `@codegraphy-dev/plugin-svelte` to analyze `.svelte` components with TypeScript. The plugin uses `svelte/compiler` to parse component structure and extract module and instance scripts from the Svelte abstract syntax tree (AST). It emits static import, type-import, and dynamic `import()` Relationships between Svelte components, shared models, TypeScript entrypoints, and loaded feature modules. A default `**/.svelte-kit/**` filter excludes generated SvelteKit files until users disable the filter.
