...
|---------|---------------------------------------|
| `example-typescript` | `src/index.ts` imports `formatDecision`, calls `evaluateCheckout`, type-imports `RolloutRequest`, and declares `demoRequest`; `PercentageEvaluator` extends `BaseEvaluator` and implements `FlagEvaluator` for TypeScript inheritance coverage. |
