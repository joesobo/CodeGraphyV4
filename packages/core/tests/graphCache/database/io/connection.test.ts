...
describe('graphCache/database/io/connection', () => {
  it('executes statements and reads deterministic rows', () => {
    const rows = withConnection(createDatabasePath(), (connection) => {
      runStatementSync(
        connection,
        "INSERT INTO File(path, analysisPath, mtime, size, nodesIndexed, symbolsIndexed, relationsIndexed, cacheTiersIndexed) VALUES ('src/app.ts', '/workspace/src/app.ts', 1, 2, 0, 0, 0, 0)",
      );
      return readRowsSync(
        connection,
