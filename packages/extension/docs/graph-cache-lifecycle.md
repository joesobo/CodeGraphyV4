...
The Extension's workspace watchers remain cache-active while the Graph View is closed. Closed-view event batches use the Core-owned workspace cache updater, which initializes from the complete persisted cache, serializes incremental work, and flushes pending changes when the Extension releases it. When the Graph View is open, the Extension analysis pipeline owns both the visible update and persistence; transitioning back to it first releases the closed-view updater so two in-memory engines do not remain active for the workspace. Operation-scoped Graph Cache write coordination also prevents an independent CLI watcher from racing the Extension's SQLite persistence.
