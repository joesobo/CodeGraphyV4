  ...
  it('extracts PHP use relationships, simple inheritance, and useful symbols', async () => {
    ...
    expect(result?.relations).toEqual(expect.arrayContaining([
      expect.objectContaining({
        ...
        target: { kind: 'file', path: path.join(workspaceRoot, 'src/App/Base/BaseRunner.php'), pathKind: 'absolute', specifier: 'App\\Base\\BaseRunner' },
      }),
      expect.objectContaining({
        edgeType: 'import',
        pluginId: 'codegraphy.treesitter',
        sourceId: 'codegraphy.treesitter:import',
        from: { kind: 'file', filePath: runnerPath },
        target: { kind: 'unresolved', specifier: 'Vendor\\Package\\Thing' },
      }),
      expect.objectContaining({
        edgeType: 'inherit',
        pluginId: 'codegraphy.treesitter',
        ...
        from: { kind: 'file', filePath: runnerPath },
        target: { kind: 'file', path: path.join(workspaceRoot, 'src/App/Base/BaseRunner.php'), pathKind: 'absolute', specifier: 'BaseRunner' },
      }),
      expect.objectContaining({
        edgeType: 'inherit',
        pluginId: 'codegraphy.treesitter',
        sourceId: 'codegraphy.treesitter:inherit',
        from: { kind: 'file', filePath: runnerPath },
        target: { kind: 'file', path: path.join(workspaceRoot, 'src/App/Contracts/Runnable.php'), pathKind: 'absolute', specifier: 'Runnable' },
      }),
    ]));
