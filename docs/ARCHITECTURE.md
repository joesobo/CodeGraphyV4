...
**`App.tsx`** listens for `ExtensionToWebviewMessage` events, manages all UI state, and renders the graph with panels. The toolbar includes an Export dropdown powered by `@radix-ui/react-dropdown-menu`, split into labeled sections: `Images` (PNG, SVG, JPEG) and `Connections` (JSON, Markdown). Export actions are requested through `Graph.tsx` so all formats use the same currently-rendered graph snapshot (including timeline commit state). It also hosts the Tier-2 runtime (`WebviewPluginHost`), handles `PLUGIN_WEBVIEW_INJECT`, dynamically loads plugin scripts/styles, and routes plugin-scoped messages.
...
**`lib/exportData.ts`** builds the structured connections export payload (`format: codegraphy-export`, `version: 2.0`). Output is organized into labeled sections optimized for agent parsing: `sections.connections` (rules, grouped files, ungrouped files) and `sections.images` (image path to owning groups), plus `scope.timeline` metadata for commit-aware exports.

**`lib/exportMd.ts`** renders a human-readable Markdown export from the same structured export data, preserving section labels (`Connections`, `Images`) and timeline scope metadata.
