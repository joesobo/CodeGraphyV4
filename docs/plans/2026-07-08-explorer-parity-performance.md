...
2. **Obsidian graph view** — contextualized against public staff/community reports (~25,000-file practical ceiling and large-vault single-core saturation). A historical user report observed 8–20% idle CPU, while an Obsidian team reply says rendering should stop after about one second; CodeGraphy's own <2% gate remains authoritative. Sources and caveats are in the research appendix.
...
- A distinct TDD experiment stabilized filter-popover callbacks and memoized `Graph`, skipping an immediate stale-projection parent render while still rerendering for the real projection/revision. Twenty-five focused tests passed. Its supported `large` sequence completed with both hosts exiting 0 and node:file pair maxima **656.23 / 635.72 / 112.31 / 58.10 / 67.53ms** (median **112.31ms**), zero layout resets, and zero settles. The median regressed **19.2%** from the exact control, so the callback/memo changes and tests were reverted and no baseline was adopted. The next distinct attempt must target projection acknowledgement rather than retained Graph render cost.
- The third distinct attempt lets an exact scope-toggle commit publish after React commits when the real layout is unchanged, scope visibility is present, and no earlier frame is pending; layout-changing and non-scope work keep the next-frame path. Thirty-six focused commit/lifecycle/projection tests passed. Its supported `large` run completed with both hosts exiting 0 and node:file pair maxima **517.71 / 479.45 / 79.87 / 52.64 / 61.54ms** (median **79.87ms**), zero layout resets, and zero settles: **15.2% faster** than the exact **94.19ms** control, so the improvement is accepted. The 50ms speed target still fails; after three distinct attempts the remaining large warm-scope latency is a recorded blocker, while the stable zero-reset contract remains green.
- Phase 1 exact-head reconciliation checked only emit-site and scenario coverage complete. `payloadBytes` and `layoutResets` are now reduced from the correlated single-save operation (`3fcd5c8ed`), and active fixture tests stop at `huge --symbols` (`df342b293`). Remaining blockers: fixture-wide single-window execution/diagnostics-bus conformance; no public VS Code Explorer paint-completion event for truly equivalent visible spans; release-bundle perf scaffolding deferred to Phase 9; exact-head five-run stability plus a real sleep-500 CI proof; and the manual workflow's legacy giant matrix cannot be pushed until the GitHub credential has `workflow` scope (PR comment records the rejected push).
...
- **Idle cost:** a historical user report observed 8–20% idle CPU with a graph pane open, but an Obsidian team reply in the same thread says graph rendering should stop about one second after interaction and asks affected users for a performance trace (https://forum.obsidian.md/t/graph-obsidian-consistently-eating-8-20-cpu-at-idle/2349/14). Treat this as anecdotal context, not a current product guarantee. CodeGraphy's own <2% gate remains the acceptance contract; gates 3-F/3-J target idle and interaction cost directly.
...
- [x] **2.2 Cut/Copy/Paste:** `extension/actions/clipboardFiles.ts`; pure `resolveCollisionName(name, siblings)` implementing V4 exactly (unit-test the sequence ` copy`, ` copy 2`, ` copy 3`); V2 error shape; V8 count-confirm; wire contracts + `node/entries.ts` + `node/folderEntries.ts` + `background/entries.ts` + `providerMessages/primaryActions/workspaceFileActions.ts`. Acceptance first, then implement, then quality-tool loop. Update Trello card. Commits `0e33eb58f` and `058246d24` implement the undoable host actions, runtime validation, decision-aware menus, and keyboard bindings with focused coverage.

Cut/copy/paste acceptance checkpoint (2026-07-11): the runtime-validated, undoable host actions and keyboard/context-menu wiring retain the focused V2/V4/V8 coverage. A dedicated real VS Code **1.128.0** Playwright walkthrough passed in **5.1s**: keyboard Copy/Paste created `dest/a.ts`, the repeated paste collision created `dest/a copy.ts`, and Cut/Paste moved `src/b.ts` to `dest/b.ts`; `clipboard-files-complete.png` is attached to the test artifact. The human-owned Gherkin files were not modified. The linked Trello card `aB159IyL` contains the PR/evidence comments and is in Done.

Nested-create acceptance checkpoint (2026-07-11): the toolbar's production New File path accepted `nested/deep/example.ts`, created every missing parent directory, and published the new node into the live graph. The real VS Code **1.128.0** Playwright gate passed in **3.8s** and attached `nested-create-complete.png`. This completes the explicit V3 Dev Host proof; Task 2.1 remains open for the rest of the all-done-row walkthrough.
...
### Phase 4 implementation checkpoint and speed blocker (2026-07-11)

- The loaded graph is the webview-owned evidence superset. Pure shared predicates project node/edge masks locally; `pnpm boundaries` reported zero layer violations (the command remains nonzero because of five pre-existing dead surfaces and eleven dead ends). A weak evidence index is built once per graph identity with node-kind IDs, edge-kind IDs, and symbol→file ownership. No-op masks retain the exact graph object, and newly revealed symbols inherit their containing file's position while retained runtime objects keep their physics state.
- First-time cached symbol evidence now diff-patches the already loaded graph in bounded 500-item, remove-before-add chunks with an event-loop yield between messages. The host remembers hydrated cache tiers, so later off/on toggles do not reread Graph Cache or resend graph data. A typed hydration-state message drives a disabled row-level spinner until the initial stream completes.
- Adjacent Timeline Snapshots send `diffGraphData` patches instead of full `COMMIT_GRAPH_DATA` graph payloads. The webview applies those patches in place without incrementing `graphResetVersion`, preserving live object identity.
- Graph Revision relation changes now materialize as a default-hidden `revision:diff` evidence class with `added`/`removed` metadata. Removed relations are retained only when both endpoints exist in the selected revision; stale evidence is replaced at every revision boundary. The ordinary Graph Scope edge mask toggles this layer entirely in the webview. The adjacent-revision fixture emitted an **831-byte** patch for a **30,786-byte** evidence graph (**2.70%** of a full replay) and stayed under the gate's modeled **2,560-byte** changed-file allowance. Ten focused host/default/projection tests pass.
- The scope battery's waiter now records the exact applied projection without serializing the latency metric on physics settlement. One complete real-VS-Code `medium` battery recorded 180 measured toggles, zero measured `payloadBytes`, zero `layoutResets`, and one unmeasured cold-hydration `cacheSaveMs`. Warm medians still failed 4-B: 34/36 row-directions exceeded 50ms; the slowest was `node:symbol:constant:disabled` at `292.739ms`.
- Three additional isolated attempts—no-op evidence identity/indexing, a 16ms projection-frame fallback, and synchronous external-store commit plus bounded perf-commit correlation—did not change the governing shape. The final one-repetition diagnostic still had 35/36 directions above 50ms (slowest `253.510ms`). Per the plan's three-attempt rule, the warm-speed thread remains a recorded blocker and work continues on independent phases. The required five-run `large` and `huge --symbols` acceptance battery is therefore not claimed.
...
Gate 6-B blocker checkpoint (2026-07-11, three retained attempts): attempt 1 measured the inline collision but exposed a duplicate global mutation toast; attempt 2 removed the duplicate and exposed rollback refresh clearing the recovered editor; attempt 3 preserved the editor and passed the appearance/collision assertions, but the host's redundant native `showErrorMessage` stole focus from the webview. The provider now gives named canvas mutations exclusive ownership of inline failure presentation, but the zero-focus-departure assertion is not accepted until a later fresh desktop run validates it. No failed run was discarded or reported as passing.

Fresh 6-B validation (2026-07-11): the exact-head Dev Host still reported the mounted rename input as inactive after the collision. A focused refocus-on-error experiment passed its webview unit test but the next real-window diagnostic reported the input inactive before the initial edit, so the experiment was reverted. Gate 6-B remains blocked on real VS Code focus ownership rather than missing inline state or error rendering.
...
- Task 6.3 is complete: settled graph positions and evidence are versioned into webview state, restored as a dim graph before bootstrap, and reconciled with the first live payload without incrementing `graphResetVersion` (including the structurally-identical fast path). The persistence/bootstrap pack passes 60/60 focused tests, and the real VS Code t+100ms pixel gate passed as recorded above.
...
- [x] **7.2 Manifest + compatibility:** manifest schema + load-time validation + clean refusal path; acceptance: stale-`minCoreVersion` plugin refuses politely.
- [x] **7.3 Activation discipline:** lazy activation by declared file globs/graph events; per-plugin `pluginActivationMs` metric wired into `pnpm perf`; first-party plugins migrated.
- [x] **7.4 Sample plugin template + docs:** `examples/sample-plugin` + `docs/plugins/GETTING_STARTED.md` (scaffold → watch → see node in Dev Host graph); the doc's step count is a gate.
- [x] **7.5 Dogfood — Material Icon Theme as plugin:** extract using only public plugin-api (the sufficiency proof); default-enabled; visual regression via Playwright. Update the Ideas card.
- [x] **7.6 Crash isolation:** plugin throw during analysis/menu/render contribution → plugin disabled + notification, graph unaffected; acceptance test with a deliberately-throwing fixture plugin.
...
Sample-plugin checkpoint (2026-07-11): `examples/sample-plugin` and the five-step Getting Started guide build, typecheck, and pass their focused tests; the CLI path was rehearsed in an isolated home and successfully linked, enabled, listed, and disabled the package. That rehearsal exposed and fixed the previously missing Core projection of plugin-returned custom analysis nodes; a direct real-package Core index produces the `demo.sample` file, `sample:marker` child, and containment edge. The retained Dev Host failures exposed the stale save-progress prerequisite and the missing Core projection. On the current exact head, the live graph contained both nodes; the last failure was only an invalid DOM-label locator for a canvas node. The corrected accessible-node assertion passed in real VS Code **1.128.0** in **3.5s**, and the captured window visibly showed `demo.sample` plus the orange **Hello from Sample Plugin** child. Gate 7-D passes.

Activation checkpoint (2026-07-11): package eligibility is decided from static `supportedExtensions` before module import; empty/wildcard manifests activate on the graph event, while language packages activate only when VS Code's workspace-scoped file search finds a declared extension. Import/factory timing emits `pluginActivationMs` through the Core diagnostic stream and `pnpm perf` preserves a deterministic per-plugin map. The first retained real small-fixture smoke exposed TypeScript at **146.85ms**; moving TypeScript, Svelte, and Vue compiler paths behind split ESM analysis chunks reduced the accepted real VS Code measurements to Markdown **0.12ms** and TypeScript **1.00ms**. A fresh-process sweep measured Godot **5.93ms**, Particles **0.28ms**, Svelte **0.19ms**, TypeScript **0.32ms**, Unity **0.36ms**, and Vue **0.16ms**. All are below the **50ms** gate; the small workspace activated only Markdown and its matching TypeScript plugin.

Crash-isolation checkpoint (2026-07-11): the extension registry now unregisters and disposes a plugin after an analysis or Graph View contribution boundary throws, reports one notification, and continues with healthy analysis facts/contributions. Menu label/visibility/run callbacks and webview node/overlay/tooltip renderers are guarded; a webview failure removes local registrations and persists the plugin as disabled. The deliberately throwing package fixture passed in real VS Code **1.128.0** in **3.8s**: the notification was visible, `demo.throw` remained in the graph, and the graph controls stayed usable. Focused host/webview packs cover analysis, menu, and render failures.

Material-icons dogfood checkpoint (2026-07-11): explorer-style file/folder icon matching now lives in the default-enabled `@codegraphy-dev/plugin-material-icons` package, owns its `material-icon-theme` dependency, and consumes only `@codegraphy-dev/plugin-api`. Plugin API **2.2.0** adds graph-aware declarative `graphView.defaultGroups`; the extension adapts those public rules while preserving stable legend IDs and owns no Material theme implementation. The migrated package suite passes **51/51**, focused host integration passes **26/26**, extension and package typechecks are clean, and focused `pnpm boundaries -- plugin-material-icons/` reports **0 layer violations, 0 dead surfaces, and 0 dead ends**. The repo-wide scan also reports zero layer violations but remains nonzero on its pre-existing dead-end inventory. A real VS Code **1.128.0** Playwright visual gate passed in **3.5s**, proving the bundled plugin supplies a loaded `package.json` SVG to both the rendered graph node and Themes legend. The absorbed Trello Ideas card `sz2128BX` now contains the PR/evidence note and is in Done.
