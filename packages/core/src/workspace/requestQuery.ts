...
function shouldApplyWorkspaceGraphScope(input: WorkspaceGraphQueryInput): boolean {
  ...
  if (input.report === 'search' || input.report === 'triage' || input.report === 'overview' || input.report === 'paths') return false;
...
function executeWorkspaceGraphQuery(
  ...
  const sourceText = input.report === 'search' || input.report === 'triage'
