...
vi.mock('./acceptance/graphView/canvas', async (importOriginal) => {
  ...
  return {
    ...actual,
    clickNode: vi.fn(),
    expectVisibleEdgeBetween: vi.fn(),
...
describe('acceptance graph view step resolution', () => {
  ...
  it('uses an existing TypeScript node when the acceptance phrase selects multiple nodes', async () => {
    const { graphViewAcceptanceSteps } = await import('./acceptance/graphView/steps');
    const { clickNode, modifierClickNode, rightClickNode } = await import('./acceptance/graphView/canvas');
    const text = 'I click and drag on the background I can select multiple nodes at once';
    ...
    await implementation({ exampleName: 'example-typescript' } as GraphAcceptanceContext, {
      keyword: 'When',
      line: 1,
      sourcePath: 'test.md',
      text,
    ...
    expect(rightClickNode).not.toHaveBeenCalled();
    expect(clickNode).toHaveBeenCalledWith(expect.anything(), 'src/index.ts');
    expect(modifierClickNode).toHaveBeenCalledWith(expect.anything(), 'src/rollout.ts');
  });

  it('routes graph rendering edge phrases with their source and target paths', async () => {
    const { graphViewAcceptanceSteps } = await import('./acceptance/graphView/steps');
    const { expectVisibleEdgeBetween } = await import('./acceptance/graphView/canvas');
    const text = 'the src/index.ts node has an edge that points to the src/format.ts node';
    ...
    expect(expectVisibleEdgeBetween).toHaveBeenCalledWith(expect.anything(), 'src/index.ts', 'src/format.ts');
  });
