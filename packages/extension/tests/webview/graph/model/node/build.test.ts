...
describe('graph/model/node/build', () => {
  ...
  it('marks owned members without synthesizing Organize Graph Section nodes', () => {
    const nodes = buildGraphNodes({
      nodes: [
    ...
    expect(nodes.map(node => node.id)).toEqual(['src/app.ts']);
    expect(nodes.some(node => node.isGraphSection)).toBe(false);
  });
